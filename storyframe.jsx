...
export default function StoryFrame() {
  ...
  const handleSubjectDragStart = useCallback((e) => {
    e.stopPropagation();
    if (e.touches && e.touches.length === 2) return;
    e.preventDefault();
    subjectDragging.current = true;
    const cx = e.clientX ?? e.touches?.[0]?.clientX;
    const cy = e.clientY ?? e.touches?.[0]?.clientY;
    subjectDragStart.current = { mx: cx, my: cy, px: subjectPosRef.current.x, py: subjectPosRef.current.y };
  }, []);

  const handleSubjectDragMove = useCallback((e) => {
    if (!subjectDragging.current) return;
    const cx = e.clientX ?? e.touches?.[0]?.clientX;
    const cy = e.clientY ?? e.touches?.[0]?.clientY;
    const nx = subjectDragStart.current.px + (cx - subjectDragStart.current.mx);
    const ny = subjectDragStart.current.py + (cy - subjectDragStart.current.my);
    subjectPosRef.current = { x: nx, y: ny };
    setSubjectPos({ x: nx, y: ny });
  }, []);
    ...
    try {
      ...
      if (s.mainDataUrl) {
        ...
        if (s.popOutEnabled && s.subjectUrl) {
          ...
          // Layer 3: Subject — scaled + positioned independently, only OUTSIDE frame visible
  ...
  const fso = (() => {
    ...
    if(frame==="polaroid")  return {...b, background:"#fff", colorScheme:"only light", padding:"3% 3% 4% 3%",  borderRadius:3};
    if(frame==="rounded")   return {...b, background:"#fff", colorScheme:"only light", padding:"2%",            borderRadius:16};
    if(frame==="filmstrip") return {...b, background:"#fff", colorScheme:"only light", padding:"7% 3% 7% 3%",  borderRadius:0};
    ...
    <div style={{ minHeight:"100vh", background:"#080810", color:"#d3d3d3", fontFamily:"'Inter',system-ui,sans-serif", display:"flex", flexDirection:"column" }}>
      ...
      <div style={{ flex:1, display:"flex", overflow:"hidden" }}>
        ...
        <div className="sf-preview-area" style={{ flex:1, display:"flex", alignItems:"center", justifyContent:"center", padding:24, minWidth:0 }}>
          ...
          {/* Subject overlay — fully independent layer: own drag + pinch */}
          {popOutEnabled && subjectUrl && mainDataUrl && frame !== "none" && (
            <div
              style={{
                position:"absolute", left:"50%", top:"50%",
                width:`${scale}%`,
                transform:`translate(calc(-50% + ${subjectPos.x}px), calc(-50% + ${subjectPos.y}px)) scale(${subjectScale/100})`,
            ...
            >
              <img src={subjectUrl} alt="" style={{ display:"block", width:"100%", height:"auto", pointerEvents:"none" }} />
            </div>
          )}
          </div>{/* end wrapper */}
        </div>
      ...
      <div className="sf-mobile" style={{ position:"fixed", bottom:0, left:0, right:0, zIndex:100, background:"transparent", borderTop:"1px solid rgba(255,255,255,0.1)" }}>
        ...
        {mobileTab && (
          <div style={{ padding:"12px 16px 8px", borderBottom:"1px solid rgba(255,255,255,0.05)", overflowY:"auto", maxHeight:200, background:"rgba(12,12,24,0.5)" }}>
